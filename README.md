...
Although the array multiplier functions correctly, it is not suitable for inclusion inside a single-cycle ALU.
Array multipliers have a very long combinational delay, and placing them inside the ALU would dramatically increase the ALU’s critical path. This reduces the maximum operating frequency of the entire processor.
For this reason, real CPU designs implement multiplication in a separate hardware unit—often multi-cycle or pipelined—while keeping the ALU optimized for fast operations like addition, subtraction, logic, and shifts.
